#!/usr/bin/env python3
"""Build MERGED_LIBRARIES_README.md from repo sources + generated API catalog."""
from pathlib import Path

ROOT = Path(__file__).resolve().parent
OUT = ROOT / "MERGED_LIBRARIES_README.md"
CAT = ROOT / "_api_catalog_generated.md"

HEADER = r'''# Merged Platform README — PicoGK + ShapeKernel + LatticeLibrary + Godot
...
def main():
    lines = [HEADER]
    if CAT.exists():
        cat = CAT.read_text(encoding='utf-8', errors='replace').splitlines()
        # Cap appendix so total ~1200-1250 lines
        max_appendix = 1200 - len(lines)
        if len(cat) > max_appendix:
            cat = cat[: max_appendix - 3] + ['', '*(Appendix truncated — full catalog has 2100+ lines; re-run `_build_merged_readme.py` after API changes.)*', '']
        lines.extend(cat)
    else:
        lines.append('*Run `python _build_merged_readme.py` to regenerate appendix.*')
    lines.append('')
    lines.append('---')
    lines.append('')
    lines.append('*End of Merged Platform README.*')
